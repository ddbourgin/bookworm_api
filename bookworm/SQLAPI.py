...
class userquery:
	...
	# change the database if the phoneme mode is selected in the UI
	if 'searchmode' in outside_dictionary.keys() and outside_dictionary['searchmode'] == 'Phonemes':
	    self.prefs['database'] = self.prefs['database'] + '_phonemes'
	...
	self.searchmode = outside_dictionary.setdefault('searchmode',"Words")

        lookups = {"Case_Insensitive":'word','lowercase':'lowercase','casesens':'casesens',"case_insensitive":"word","Case_Sensitive":"casesens","All_Words_with_Same_Stem":"stem",'stem':'stem','regex':'word'}
        self.word_field = lookups[self.words_collation]

        # add a flag for regex searches
	self.regex = False
	if self.words_collation == 'regex':
	    self.regex = True
	...
	self.time_limits = outside_dictionary.setdefault('time_limits',[0,10000000])
        ...
        for group in groups:
            ...
            multigramSearch = re.match("(unigram|bigram|trigram|four_gram|five_gram|six_gram)(\d)?",group)
...
#            self.relevantTables.remove('catalog')
        ...
        for table in self.relevantTables:
            if table!="fastcat" and table!="words" and table!="wordsheap" and table!="master_bookcounts" and table!="master_bigrams" and table!="master_trigrams" and table!="master_fourgrams" and table!="master_fivegrams" and table!="master_sixgrams":
    ...
    def make_catwhere(self):
        ...
        for key in self.limits.keys():
            ...
            if key not in ('word','word1','word2','word3','word4','word5','word6','hasword') and not re.search("words\d",key):
	...
	self.matches = []
        for gramterm in ['unigram','bigram','trigram','fourgram','fivegram','sixgram']:
        ...
        if 'word' in self.limits.keys():
		    ...
		    self.regex = False
		    for cc in '^$.+*|[]{}':
			if cc in searchingFor:
			    self.regex = True
			    break

		    # this is where we use the regex flag. Note that this often causes response highlighting to break
		    if self.regex:
                        selectString =  "SELECT wordid,word FROM wordsheap WHERE %s REGEXP '%s'" %(self.word_field,searchingFor)
		    else:
			selectString = "SELECT wordid,word FROM wordsheap WHERE %s = '%s'" %(self.word_field,searchingFor)
    ...
    def build_wordstables(self):
        ...
        """
	needsSixgrams = (self.max_word_length == 6 or re.search("words6",self.selections))
	needsFivegrams = (self.max_word_length == 5 or re.search("words5",self.selections))
	needsFourgrams = (self.max_word_length == 4 or re.search("words4",self.selections))
	needsTrigrams = (self.max_word_length == 3 or re.search("words3",self.selections))
        ...
        if needsSixgrams:
            ...
            self.maintable = 'master_sixgrams'

            self.main = '''
                 JOIN
                 master_sixgrams as main
                 ON ('''+ self.prefs['fastcat'] +'''.bookid=main.bookid)
                 '''

            self.wordstables =  """
            JOIN %(wordsheap)s as words1 ON (main.word1 = words1.wordid)
            JOIN %(wordsheap)s as words2 ON (main.word2 = words2.wordid) 
	    JOIN %(wordsheap)s as words3 ON (main.word3 = words3.wordid)
	    JOIN %(wordsheap)s as words4 ON (main.word4 = words4.wordid)
	    JOIN %(wordsheap)s as words5 ON (main.word5 = words5.wordid)
	    JOIN %(wordsheap)s as words6 ON (main.word6 = words6.wordid)""" % self.__dict__

        elif needsFivegrams:

            self.maintable = 'master_fivegrams'

            self.main = '''
                 JOIN
                 master_fivegrams as main
                 ON ('''+ self.prefs['fastcat'] +'''.bookid=main.bookid)
                 '''
	    ...
	    JOIN %(wordsheap)s as words3 ON (main.word3 = words3.wordid)
	    JOIN %(wordsheap)s as words4 ON (main.word4 = words4.wordid)
	    JOIN %(wordsheap)s as words5 ON (main.word5 = words5.wordid)""" % self.__dict__

        elif needsFourgrams:

            self.maintable = 'master_fourgrams'

            self.main = '''
                 JOIN
                 master_fourgrams as main
                 ON ('''+ self.prefs['fastcat'] +'''.bookid=main.bookid)
                 '''
            ...
            JOIN %(wordsheap)s as words2 ON (main.word2 = words2.wordid) 
	    JOIN %(wordsheap)s as words3 ON (main.word3 = words3.wordid)
	    JOIN %(wordsheap)s as words4 ON (main.word4 = words4.wordid)""" % self.__dict__

        elif needsTrigrams:

            self.maintable = 'master_trigrams'

            self.main = '''
                 JOIN
                 master_trigrams as main
                 ON ('''+ self.prefs['fastcat'] +'''.bookid=main.bookid)
                 '''
            ...
            JOIN %(wordsheap)s as words1 ON (main.word1 = words1.wordid)
            JOIN %(wordsheap)s as words2 ON (main.word2 = words2.wordid) 
	    JOIN %(wordsheap)s as words3 ON (main.word3 = words3.wordid)""" % self.__dict__
