...
#            self.relevantTables.remove('catalog')
        ...
        for table in self.relevantTables:
            if table!="fastcat" and table!="words" and table!="wordsheap" and table!="master_bookcounts" and table!="master_bigrams" and table!="master_trigrams" and table!="master_four_grams" and table!="master_five_grams" and table!="master_six_grams":
    ...
    def make_wordwheres(self):
        ...
        for gramterm in ['unigram','bigram','trigram','four_gram','five_gram','six_gram']:
        ...
        if 'word' in self.limits.keys():
		    ...
		    if self.regex:
                        selectString =  "SELECT wordid FROM wordsheap WHERE %s REGEXP '%s'" %(self.word_field,searchingFor)
		    else:
	...
	needsTrigrams = (self.max_word_length == 3 or re.search("words3",self.selections))
        ...
        if needsSixgrams:
            ...
            self.maintable = 'master_six_grams'

            self.main = '''
                 JOIN
                 master_six_grams as main
	    ...
	    JOIN %(wordsheap)s as words6 ON (main.word6 = words6.wordid)""" % self.__dict__
            ...
            self.maintable = 'master_five_grams'

            self.main = '''
                 JOIN
                 master_five_grams as main
                 ON ('''+ self.prefs['fastcat'] +'''.bookid=main.bookid)
                 '''
	    ...
	    JOIN %(wordsheap)s as words5 ON (main.word5 = words5.wordid)""" % self.__dict__
            ...
            self.maintable = 'master_four_grams'

            self.main = '''
                 JOIN
                 master_four_grams as main
                 ON ('''+ self.prefs['fastcat'] +'''.bookid=main.bookid)
                 '''
