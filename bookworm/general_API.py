...
class APIcall(object):
    ...
    def execute(self):
        ...
        if method=="return_tsv" or method=="tsv":
            import csv
            frame, matches = self.data()
            return frame.to_csv(sep="\t",encoding="utf8",index=False,quoting=csv.QUOTE_NONE,escapechar="\\"), matches

        if method=="return_pickle" or method=="DataFrame":
            frame, matches = self.data()
            from cPickle import dumps as pickleDumps
            return pickleDumps(frame,protocol=-1), matches
        ...
        if method in ["returnPossibleFields","search_results","return_books"]:
            ...
            return json.dumps(query.execute()), matches
    ...
    def multi_execute(self):
	    ...
	    data, matches = self.__class__(child).return_json(raw_python_object=True)
	    returnable.append(data)
	    matchez.append(matches)
#            returnable.append(self.__class__(child).return_json(raw_python_object=True))
        return json.dumps(returnable), matchez

    def return_json(self,raw_python_object=False):
        query = self.query
        data, matches = self.data()
        ...
        if raw_python_object:
            return returnt, matches

        try:
            return json.dumps(returnt,allow_nan=False), matches
        except ValueError:
            return json.dumps(returnt), matches
